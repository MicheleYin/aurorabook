...
use crate::hooks::use_library;
use crate::hooks::use_toast::ToastProvider;
use crate::hooks::use_conversion::ConversionProvider;
use crate::hooks::use_reader_state::ReaderStateProvider;
...
pub fn App() -> impl IntoView {
    ...
    // Load library at app level so it persists across tab changes
    let (library, is_loading_library, refresh_library) = use_library();
    provide_context(library);
    provide_context(is_loading_library);
    provide_context(refresh_library);
    ...
    view! {
        <ToastProvider>
            <ConversionProvider>
                <ReaderStateProvider>
                    <AppContentWithNavigation active_view=active_view.into() set_active_view=set_active_view />
                </ReaderStateProvider>
            </ConversionProvider>
        </ToastProvider>
    }
...
#[component]
fn AppContentWithNavigation(
    active_view: Signal<ActiveView>,
    set_active_view: WriteSignal<ActiveView>,
) -> impl IntoView {
    // Auto-navigate to reader when a book is opened, and back to library when book is cleared
    let reader_state = crate::hooks::use_reader_state::use_reader_state();
    create_effect(move |_| {
        let active_book_id = reader_state.get_active_book_id().get();
        let current_view = active_view.get();
        if active_book_id.is_some() && current_view != ActiveView::Reader {
            set_active_view.set(ActiveView::Reader);
        } else if active_book_id.is_none() && current_view == ActiveView::Reader {
            set_active_view.set(ActiveView::Library);
        }
    });
