...
private final class BackgroundRuntimeBridge {
    ...
    func register(_ entries: [RegistrationEntry]) throws {
        for entry in entries {
            ...
            // Continued-processing wildcards (….*) belong in Info.plist only.
            // Apple requires registering the concrete submitted id at submit-time.
            if kind == taskKindContinuedProcessing, identifier.hasSuffix(".*") {
                continue
            }

            try registerConcreteTask(identifier: identifier, kind: kind)
        }
    }

    func registerConcreteTask(identifier: String, kind: UInt8) throws {
        // Continued processing launch handlers should run on the main queue (Apple samples / forums).
        let queue: DispatchQueue? = (kind == taskKindContinuedProcessing) ? .main : nil
        let registered = BGTaskScheduler.shared.register(forTaskWithIdentifier: identifier, using: queue) {
            [weak self] task in
            guard let self else {
                return
            }
