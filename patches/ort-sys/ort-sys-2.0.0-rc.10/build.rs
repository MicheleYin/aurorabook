...
fn copy_libraries(lib_dir: &Path, out_dir: &Path) {
    ...
    for out_dir in [
        ...
        let lib_files = std::fs::read_dir(lib_dir).unwrap_or_else(|_| {
            panic!(
...
fn static_link_prerequisites(using_pyke_libs: bool) {
    ...
    let cpp_link_stdlib =
        if let Ok(stdlib) = env::var(ORT_ENV_CXX_STDLIB).or_else(|_| env::var(ENV_CXXSTDLIB)) {
            if stdlib.is_empty() {
                None
            } else {
                Some(stdlib)
...
fn prepare_libort_dir() -> (PathBuf, bool) {
    if let Ok(base_lib_dir) = env::var(ORT_ENV_SYSTEM_LIB_LOCATION) {
        ...
        #[cfg(feature = "download-binaries")]
        {
            ...
            if dist.is_none() {
                // Check if we're doing direct linking (libraries linked in build.rs)
                // If ORT_DIRECT_LINK is set, skip the panic and return early
                if env::var("ORT_DIRECT_LINK").is_ok() {
                    println!("cargo:warning=Direct linking enabled, skipping download check");
                    // Return empty path - libraries are linked directly in build.rs
                    return (PathBuf::default(), false);
                }

                // Also check if libraries exist in common build locations (for iOS)
                if target.contains("apple-ios") {
                    ...
                    for ort_lib_dir in possible_paths {
                        if ort_lib_dir.exists()
                            && ort_lib_dir.join("libonnxruntime_common.a").exists()
                        {
                            println!("cargo:warning=Found ONNX Runtime libraries at {} for direct linking, skipping download", ort_lib_dir.display());
                            // Return empty path - libraries are linked directly in build.rs
                            return (PathBuf::default(), false);
                        }
                    }
            ...
            if !lib_dir.exists() {
                ...
                assert!(
                    verify_file(&downloaded_file, prebuilt_hash),
                    "hash of downloaded ONNX Runtime binary does not match!"
                );
...
fn try_setup_with_pkg_config() -> bool {
    match pkg_config::Config::new().probe("libonnxruntime") {
        Ok(lib) => {
            let expected_minor = ONNXRUNTIME_VERSION
                .split('.')
                .nth(1)
                .unwrap()
                .parse::<usize>()
...
fn real_main(link: bool) {
    ...
    let lib_dir = if install_dir.join("lib").exists() {
    ...
    if link {
        if needs_link {
            ...
            let static_lib_file_name = if target_os.contains("windows") {
                "onnxruntime.lib"
            } else {
                "libonnxruntime.a"
            };
