			...
			if dist.is_none() {
				...
				if target.contains("apple-ios") {
					...
					let possible_paths = vec![
						// Path relative to ort-sys crate (when patched)
						env::var("CARGO_MANIFEST_DIR").ok().map(|m| {
							Path::new(&m).parent().and_then(|p| p.parent())
								.map(|root| root.join("onnxruntime").join("build").join("iOS").join("Release").join("Release-iphoneos"))
						}).flatten(),
						// Path from project root (when building from project)
						env::var("CARGO_MANIFEST_DIR").ok().map(|m| {
							Path::new(&m).parent().and_then(|p| p.parent()).and_then(|p| p.parent())
								.map(|root| root.join("onnxruntime").join("build").join("iOS").join("Release").join("Release-iphoneos"))
						}).flatten(),
						// Absolute path check
						Some(PathBuf::from("/Users/micheleyin/Documents/tts-tauri/onnxruntime/build/iOS/Release/Release-iphoneos")),
					];
					
					for ort_lib_dir in possible_paths.into_iter().flatten() {
