...
pub async fn init_db_connection(app: &AppHandle) -> Result<(), String> {
    ...
    if let Some(parent) = db_path.parent() {
        ...
        if let Err(e) = std::fs::File::create(&test_file) {
            log::warn!(
                "Warning: Cannot write to database directory {:?}: {}",
                parent,
                e
            );
        } else {
    ...
    log::info!(
        "Database connection options created for path: {:?}",
        db_path
    );
    ...
    match crate::book_service::repositories::EpubRepository::repair_epub_file_links(&pool, app)
        .await
    {
...
pub async fn get_db_connection(_app: &AppHandle) -> Result<Arc<SqlitePool>, String> {
    ...
    let pool_arc = DB_POOL.get().ok_or_else(|| {
        "Database connection not initialized. Call init_db_connection first.".to_string()
    })?;
...
async fn migrate_epub_data_schema(pool: &SqlitePool) -> Result<(), String> {
    ...
    if let Err(e) =
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_epub_data_book_id ON epub_data(book_id)")
            .execute(pool)
            .await
    {
...
async fn migrate_live_sentence_alignment_schema(pool: &SqlitePool) -> Result<(), String> {
    ...
    sqlx::query("ALTER TABLE live_sentence_alignment__m RENAME TO live_sentence_alignment")
        ...
        .map_err(|e| {
            format!(
                "Failed to rename migrated live_sentence_alignment table: {}",
                e
            )
        })?;
...
async fn init_database_schema(pool: &SqlitePool) -> Result<(), String> {
    ...
    // Migration: conversion session timing columns for resume-aware ETA
    {
        ...
        if !books_sql.contains("conversion_session_baseline_words") {
            ...
            sqlx::query("ALTER TABLE books ADD COLUMN conversion_session_baseline_words INTEGER")
                .execute(pool)
                .await
                .map_err(|e| {
                    format!(
        ...
        if let Ok(rows) = autoindexes {
            for row in rows {
                ...
                if autoindex_name != "sqlite_autoindex_books_1" {
                    ...
                    match sqlx::query(&drop_sql).execute(pool).await {
                        Ok(_) => log::info!(
                            "Dropped autoindex {} (removed unique constraint on source_path)",
                            autoindex_name
                        ),
    ...
    let row =
        sqlx::query("SELECT sql FROM sqlite_master WHERE type='table' AND name='app_settings'")
            .fetch_one(pool)
            .await
    ...
    if !table_sql.contains("last_opened_book_id") {
        ...
        sqlx::query("ALTER TABLE app_settings ADD COLUMN last_opened_book_id TEXT")
            ...
            .map_err(|e| {
                format!(
                    "Failed to add last_opened_book_id column to app_settings: {}",
                    e
                )
            })?;
    }
    ...
    if !table_sql.contains("library_view_mode") {
        ...
        .map_err(|e| {
            format!(
                "Failed to add library_view_mode column to app_settings: {}",
                e
            )
        })?;
    }
    ...
    if !table_sql.contains("audio_player_minimized") {
        ...
        .map_err(|e| {
            format!(
                "Failed to add audio_player_minimized column to app_settings: {}",
                e
            )
        })?;
    }
    ...
    if !table_sql.contains("reader_header_visible") {
        ...
        .map_err(|e| {
            format!(
                "Failed to add reader_header_visible column to app_settings: {}",
                e
            )
        })?;
    }
    ...
    // Custom keyboard shortcut bindings (missing row = use frontend defaults).
    sqlx::query(
        r#"
    ...
    .execute(pool)
    .await
    .map_err(|e| format!("Failed to create keyboard_shortcuts table: {}", e))?;
